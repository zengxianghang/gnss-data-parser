function record = parseBestvelLine(line, verifyCrc)
%PARSEBESTVELLINE Parse one exact #BESTVELA standard ASCII line.
if nargin < 2 || isempty(verifyCrc), verifyCrc = false; end
msg = gnssparser.novatel.parseAsciiLine(line, 'BESTVELA', verifyCrc);
if numel(msg.fields) ~= 8
    error('gnssparser:BestvelParseError', ...
        'BESTVELA requires 8 body fields, got %d.', numel(msg.fields));
end
f = msg.fields;
record = struct();
...
if ~isfinite(value)
    error('gnssparser:BestvelParseError', 'Invalid %s value: %s.', label, text);
end
end
